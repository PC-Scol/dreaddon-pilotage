...
SET my.mongodb_path = :'mongodb_path';
SET my.file_prefix = 'uphf-';

drop function if exists getJsonFilePath;
create function getJsonFilePath(_file text) returns text as $$
begin
    _file := current_setting('my.mongodb_path') || '/' || current_setting('my.file_prefix') || TO_CHAR(CURRENT_DATE , 'yyyyMMdd') || '-' || _file;
    return _file;
end
$$
language plpgsql;
...
drop function if exists getDreVersionFilePath;
create function getDreVersionFilePath() returns text as $$
DECLARE
  _file varchar;
begin
    _file := current_setting('my.mongodb_path') || '/DRE_VERSION_' || TO_CHAR(CURRENT_DATE , 'yyyyMMdd');/*current_setting('my.today');*/
    return _file;
end
$$
language plpgsql;
...
-- création du schéma
CREATE SCHEMA IF NOT EXISTS mongo_piste_inscription;
...
DO $$ DECLARE
    r RECORD;
BEGIN
    FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname = 'mongo_piste_inscription') LOOP
		RAISE NOTICE 'Supprime la TABLE %', quote_ident(r.tablename);
        EXECUTE 'DROP TABLE IF EXISTS mongo_piste_inscription.' || quote_ident(r.tablename) || ' CASCADE';
    END LOOP;
END $$;



create table mongo_piste_inscription.version_instance (source_json JSONB);
DO $$
BEGIN
	EXECUTE 'COPY mongo_piste_inscription.version_instance(source_json) FROM ''' || getDreVersionFilePath() || ''' csv quote e''\x01'' delimiter e''\x02''';
END $$;


create table mongo_piste_inscription.dossiers_inscription (source_json JSONB);
DO $$
BEGIN
    EXECUTE 'COPY mongo_piste_inscription.dossiers_inscription(source_json) FROM ''' || getJsonFilePath('piste_inscription-dossiers_inscription.json') || ''' csv quote e''\x01'' delimiter e''\x02''';
END $$;


create table mongo_piste_inscription.paiements (source_json JSONB);
DO $$
BEGIN
    EXECUTE 'COPY mongo_piste_inscription.paiements(source_json) FROM ''' || getJsonFilePath('piste_inscription-paiements.json') || ''' csv quote e''\x01'' delimiter e''\x02''';
END $$;


create table mongo_piste_inscription.calendrier (source_json JSONB);
DO $$
BEGIN
    EXECUTE 'COPY mongo_piste_inscription.calendrier(source_json) FROM ''' || getJsonFilePath('piste_inscription-calendrier.json') || ''' csv quote e''\x01'' delimiter e''\x02''';
END $$;


create table mongo_piste_inscription.chemins (source_json JSONB);
DO $$
BEGIN
    EXECUTE 'COPY mongo_piste_inscription.chemins(source_json) FROM ''' || getJsonFilePath('piste_inscription-chemins.json') || ''' csv quote e''\x01'' delimiter e''\x02''';
END $$;
...
-- création du schéma
CREATE SCHEMA IF NOT EXISTS schema_ins_piste;
...
-- ****************************************************************************************************************************
-- Import de la version de Pegase dans une table schema_ins_piste.version_instance
--      (dans cet exemple, j'utilise le schema_ins_piste mais ce n'est pas forcément le plus logique)
-- ****************************************************************************************************************************

-- creation de la table au préalable
create table schema_ins_piste.version_instance (majeure varchar, mineure varchar, source_json JSONB);
-- recopie du JSON
insert into schema_ins_piste.version_instance (source_json)
select source_json from mongo_piste_inscription.version_instance;

-- extraction des valeurs
update schema_ins_piste.version_instance set majeure = source_json->>'majeure', mineure = source_json->>'mineure' ;
--select * from schema_ins_piste.version_instance;
...
/* TODO v28 : demandesPiece est vide dans le JSON ! */
/* *************
    demande_piece
*/
...
/* TODO v28 : pieces est vide dans le JSON ! */
/* *************
    inscription_pieces
*/
