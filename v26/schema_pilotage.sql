...
CREATE TABLE schema_pilotage.ins_admis AS
 SELECT admis.id AS "id",
    ...
    CASE  
		WHEN APP.id IS NOT NULL THEN APP.id::varchar(255)
		ELSE admis.id::varchar(255)
	END AS "id_apprenant",
    CASE  
    ...
    string_agg(ADMS.statut, ',') AS "statut_admission",
	CASE  
...
--   WHERE admis.id = '01669f41-6fc7-48d4-81db-d1a08ee71ef8'
...
IPAPP.temoin_neo_bachelier,IPC.adresse_fixe_code_postal,IPC.adresse_fixe_code_commune,IPC.adresse_fixe_libelle_commune,IPC.adresse_fixe_ligne1_ou_etage,IPC.adresse_fixe_ligne2_ou_batiment,IPC.adresse_fixe_ligne3_ou_voie,IPC.adresse_fixe_ligne4_ou_complement,
...
CREATE TABLE schema_pilotage.ins_apprenant AS
 ...
 (SELECT apprenant.code_apprenant AS "id",
    apprenant.code_apprenant,
    apprenant.ine AS "ine",
    apprenant.statut_ine,
    apprenant.nom_famille,
   ...
   WHERE code_apprenant NOT IN (
	   (SELECT code_apprenant FROM schema_gestion.apprenant WHERE code_apprenant IS NOT NULL GROUP BY code_apprenant)
	   UNION
	   (SELECT code_apprenant FROM schema_pilotage.ins_admis WHERE code_apprenant IS NOT NULL GROUP BY code_apprenant)
	 )
   )
;

CREATE UNIQUE INDEX ins_apprenant_id_idx ON schema_pilotage.ins_apprenant (id);
...
CREATE INDEX ins_inscription_en_cours_paiements_id_inscription_idx ON schema_pilotage.ins_inscription_en_cours_paiements (id_inscription);
...
SELECT 
	...
	NULL AS "diplome_version", /* TODO après v26 : disparu ?*/
	...
	NULL AS "parchemin_libelle_parcours_type_original", /* TODO après v26 : disparu ? */
...
--D.validite,
D.temoin_actif,D.etat,PAR.date_edition,PAR.numero_edition,ADPS.date_signature,/*PAR.libelle_parcours_type_original,*/
