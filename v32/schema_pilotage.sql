...
CREATE TABLE schema_pilotage.ref_diplome_sise AS
 SELECT DISTINCT ON (code) id,
    code,
    libelle_court,
    libelle_long,
    ...
    duree,
    id_nature_diplome,
    categorie_formation_sise,
    cite_domaine_detaille,
    secteur_disciplinaire_detaille_sise

   FROM schema_ref.diplome_sise
  ORDER BY code, date_debut_validite DESC;
...
CREATE TABLE schema_pilotage.ins_piece_demandee AS
 SELECT 
	piece_demandee.id,
	piece_demandee.caduque,
	piece_demandee.id_demande_piece,
	piece_demandee.id_apprenant,
	idt_apprenant.code_apprenant,
	
	inscription.id AS "id_inscription",
	piece_demandee.code_chemin,
	odf_objet_formation_chemin.id AS "id_objet_formation_chemin"
	
	
FROM schema_piece.piece_demandee
...
SELECT 
	...
	CASE  
		...
		WHEN DLOM.code_objet_maquette IS NOT NULL THEN AD.id||'_'||P.code||'_'||DLOM.code_objet_maquette
		ELSE AD.id||'_'||P.code
	END AS "id_edition",
	APP.id AS "id_apprenant",
	APP.code_apprenant,
	...
	D.temoin_diplome_intermediaire AS "diplome_temoin_diplome_intermediaire",
	D.grade AS "diplome_grade",
...
D.libelle_court,D.intitule,D.temoin_actif,D.etat,D.temoin_diplome_intermediaire,D.grade,
--PAR.date_edition,
...
CREATE TABLE schema_pilotage.chc_cursus AS 
	...
	LEFT JOIN schema_pilotage.idt_apprenant APP ON APP.id = CHCA.uuid::varchar
